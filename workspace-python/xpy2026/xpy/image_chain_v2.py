...
class SystemParameters:
    ...
    def __init__(self,
                 N=2048,                         # Samples for HxW of each Optical Image Generated
                 R=6.4e-3 ,                      # Radius of Optical Aperture
                 phi=0,                          # Phase of Aperture
                 wavelength=520e-9,    
                 f=1e-3,
                 dx=0.1e-6,
                 dy=0.1e-6,
                 pixel_pitch=1.5e-6,
                 t_exp=1,
                 quantum_efficiency=0.767,
                 optics_transmission=1,
                 dark_current=2.5,
                 well_depth=1e5,
                 ndynamic=256):
        
        self.N = N # Samples for HxW of each Optical Image Generated
        self.R = R # Radius of Optical Aperture
        self.phi = phi # Phase of Aperture
        self.lambda0 = wavelength
        self.focal_len = f
        self.dx = dx # detector width (Eq. 20)
        self.dy = dy # detector height
        self.p = pixel_pitch # sampling pitch (Eq. 24)
        self.t_exp = t_exp
        self.f_number = self.focal_len / (2 * self.R)
        self.eta = quantum_efficiency
        self.tau = optics_transmission
...
class Detector:
    ...
    def compute_detector_psf(self):
        ...
        x = np.linspace(-2 * self.p.dx, 2 * self.p.dx, self.p.N)
        y = np.linspace(-2 * self.p.dy, 2 * self.p.dy, self.p.N)
        X, Y = np.meshgrid(x, y)

        # Define 2D rectangle function
        mask = (np.abs(X) <= self.p.dx / 2) & (np.abs(Y) <= self.p.dy / 2)  
    ...
    def apply_detector_blur(self, L_image):
        ...
        # fft of L_image
        L_image_fft = np.fft.fft2(L_image)

        # image seen by the aperture and then the detector
        image_det = np.real(np.fft.ifftshift(np.fft.ifft2(det_otf * L_image_fft)))
    ...
    def sample(self, blurred_image):
        ...
        samp_pitch_x = int(self.p.p / self.p.dx)
        samp_pitch_y = int(self.p.p / self.p.dy)
    ...
    def compute_signal(self, L_image_lambda, wavelength):
        ...
        d = self.p.dx # assume square pixels for simplicity
...
class ImagingChain:
    ...
    def run(self):
        ...
        plt.subplot(2, 4, 3)
        plt.imshow(np.real(L_image_apt))
